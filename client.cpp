...
QSqlQueryModel * client::afficher_client()
{
    ...
    model->setQuery(" select ID_CLIENT,TO_CHAR(CIN),NOM,PRENOM,DATE_NAISSANCE,TO_CHAR(NUM_TEL),FONCTION,ADRESSE,DATE_CREATION,MAIL_CLIENT,GENRE,ETAT_CIVILE from client ");
...
QSqlQueryModel * client::rechercher_existe(QString num){
       ...
       model->setQuery(" select * from client c where CIN like '%'||'"+num+"'||'%' or ID_CLIENT like '%'||'"+num+"'||'%' or ADRESSE like '%'||'"+num+"'||'%' or MAIL_CLIENT like '%'||'"+num+"'||'%'  or NOM like '%'||'"+num+"'||'%' or PRENOM like '%'||'"+num+"'||'%' or NUM_TEL like '%'||'"+num+"'||'%' or GENRE like '%'||'"+num+"'||'%' or FONCTION like '%'||'"+num+"'||'%' or ETAT_CIVILE like '%'||'"+num+"'||'%'");
       model->setHeaderData(0,Qt::Horizontal,QObject::tr("ID_CLIENT"));
       model->setHeaderData(1,Qt::Horizontal,QObject::tr("CIN"));
       model->setHeaderData(2,Qt::Horizontal,QObject::tr("NOM"));
       model->setHeaderData(3,Qt::Horizontal,QObject::tr("PRENOM"));
